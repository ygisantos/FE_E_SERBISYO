...
const ChatbotWidget = () => {
  ...
  return (
    <>
      {/* Chat Button - Show when chat is not open OR when minimized */}
      {(!isOpen || isMinimized) && (
        <button
          onClick={handleChatButtonClick}
          className="fixed bottom-4 right-4 z-[999] bg-red-900 text-white rounded-full shadow-lg hover:bg-red-800 transition-all duration-200 flex items-center gap-2 group px-4 py-3"
        >
          <FiMessageSquare className="w-5 h-5" />
          <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 ease-in-out whitespace-nowrap text-sm">
