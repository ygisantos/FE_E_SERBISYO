...
const StatCard = ({ icon, label, value, color = "bg-white border-gray-200", trend, percentage }) => (
  <div className={`${color} flex flex-row items-center gap-4 rounded-lg p-2 border hover:shadow-lg hover:border-gray-300 hover:scale-105 hover:-translate-y-1 transition-all duration-200 cursor-pointer group`}>
    <div className="flex items-center justify-between mb-3">
      <div className="text-lg mt-2 group-hover:scale-110 transition-transform duration-200">{icon}</div>
      {typeof trend === 'number' && (
